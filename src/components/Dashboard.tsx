    ...
    >
      ...
      <div className="flex gap-2">
        {timerStatus === "idle" && !isComplete && (
          <button
            onClick={startTimer}
            className="px-4 py-1.5 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/30
        ...
        {timerStatus === "running" && (
          <button
            onClick={stopTimer}
            className="px-4 py-1.5 rounded-lg bg-[#BC002D]/10 border border-[#BC002D]/30
                       text-[#f2f2f2] text-xs font-mono tracking-wider
                       hover:bg-[#BC002D]/20 transition-colors"
          >
            Pause
          </button>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {todayReflected && (
          <span className="text-[9px] font-mono text-[#D4AF37]/60 tracking-wider">
            ● Today logged
          </span>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2">
        <div className="relative">
          <textarea
            ...
            className="w-full bg-white/5 border border-[#D4AF37]/15 rounded-lg
                       px-3 py-2 text-sm font-mono text-[#f2f2f2] placeholder-[#f2f2f2]/25
        ...
        <button
          type="submit"
          disabled={!text.trim()}
          className="self-end px-4 py-1.5 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/30
                     text-[#D4AF37] text-xs font-mono tracking-wider
                     hover:bg-[#D4AF37]/20 disabled:opacity-30 disabled:cursor-not-allowed
                     transition-colors"
        >
          記録 · Log
        </button>
      ...
      {hanseiLogs.length > 0 && (
        <ul className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-1
                        scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[#D4AF37]/20">
          <AnimatePresence initial={false}>
            {hanseiLogs.slice(0, 7).map((entry) => (
              ...
              >
                <span className="mt-1 w-1 h-1 rounded-full bg-[#D4AF37]/50 shrink-0" />
                <p className="flex-1 text-xs font-mono text-[#f2f2f2]/70 leading-relaxed">
                  {entry.text}
                </p>
                <button
                  onClick={() => deleteHansei(entry.id)}
                  className="opacity-0 group-hover:opacity-40 hover:!opacity-80
                             text-[#f2f2f2] text-xs transition-opacity"
...
function KintsugiPanel() {
  ...
  return (
    <motion.div
      className="glass rounded-2xl p-6 flex flex-col gap-4 border border-[#D4AF37]/30"
    ...
    >
      ...
      {!kintsugi.active && (
        <button
          ...
          className="w-full py-2.5 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/40
          ...
          <div className="flex gap-2">
            ...
            {kintsugi.repairProgress >= 1 && (
              <button
                ...
                className="flex-1 py-2 rounded-lg bg-[#D4AF37]/20 border border-[#D4AF37]/50
                           text-[#D4AF37] text-xs font-mono tracking-wider
                           hover:bg-[#D4AF37]/30 transition-colors"
              >
                完成 · Complete
              </button>
            )}
  ...
  return (
    <div className="flex items-center gap-1">
      {PHASES.map((p, i) => {
        ...
        return (
          ...
          >
            <div
              className={`
                ...
                ${isActive
                  ...
                  : isPast
                    ? "bg-white/5 border border-white/10 text-[#f2f2f2]/40"
            ...
            {i < PHASES.length - 1 && (
              <div className={`w-3 h-px ${isPast ? "bg-[#D4AF37]/30" : "bg-white/10"}`} />
            )}
          </div>
        );
      })}
      ...
      {currentIdx < PHASE_ORDER.length - 1 && (
        <button
          onClick={onAdvance}
          className="ml-2 px-2 py-1 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/20
                     text-[#D4AF37] text-[10px] font-mono hover:bg-[#D4AF37]/20 transition-colors"
        >
          →
        </button>
      )}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-xs text-[#f2f2f2]/50">
        {streak.current > 0 ? (
          <span className="text-[#D4AF37]">{streak.current}d</span>
        ) : (
          "—"
        )}
        {" "}streak
...
export default function Dashboard() {
  ...
  return (
    ...
    >
      {/* Three.js Enso background — full-bleed, pointer-events-none */}
      ...
      {/* Radial gradient mask over canvas */}
      <div
        className="absolute inset-0 z-[1] pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 80% 80% at 70% 50%, transparent 20%, #0B0B0C 75%)",
        }}
      />

      {/* Hero text — center-left */}
      <div className="relative z-10 flex flex-col justify-center pl-16 pr-8 gap-6
                       min-h-screen w-full lg:w-1/2">
        <motion.div
        ...
        >
          ...
          <h1 className="text-5xl md:text-6xl font-heading leading-[1.1] tracking-tight">
            ...
            <br />
            <span className="text-[#f2f2f2]/80 text-4xl md:text-5xl">Every Day</span>
          </h1>
      ...
      <div
        ref={sidebarRef}
        className="relative z-10 flex flex-col gap-4 justify-center
                   px-6 py-8 w-full lg:w-[380px] lg:min-h-screen
                   lg:border-l lg:border-[#D4AF37]/10"
      >
        {/* Vertical progress indicator */}
        <div className="absolute left-0 top-8 bottom-8 w-px bg-[#D4AF37]/8">
          <motion.div
        ...
        >
          <p className="text-[10px] font-mono text-[#f2f2f2]/25 leading-relaxed tracking-wider italic">
            "小さな改善が大きな変化を生む"
          </p>
          <p className="text-[9px] font-mono text-[#f2f2f2]/15 mt-1 tracking-widest">
